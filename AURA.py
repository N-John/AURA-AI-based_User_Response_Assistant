                ...
                if 'received' in message:
                    ...
                    for x in range(i):
                        ...
                        if contacts[x] in time_message[1]:
                            ...
                            sms.send(contacts[x] , code + ' PAYMENT RECEIVED. Hello ' + name + '. Your funds '+ prs +' to Wired networking have been received. Thank you for staying with us. FOR QUERIES CONTACT 0702374411. M')
                            time.sleep(20)
    ...
    def stored():
        ...
        try:
            ...
            for j in range(4):
                ...
                recount = rxData.decode('utf-8').split('\n')[-1]
                print(received)
                print(recount)
                if len(recount) >= 1:
                    print('333333333333333333333333333333333333333333333333333333333333')
                    received = received.replace(cmd, '')
                    received = received.replace('\n', ' ')
                    received = received.replace('OK', '')
                    print(received)
                    sms.foward('BTW YOU GOT THIS SMS ;' + received)
                    time.sleep(2)
            ...
            gsm.writedata("AT+CMGD=1,4")
            led.off()
            return 1
        
        
        except:
            print('STOPED CHECKING STORED')    
            led.off()
            return 0
...
###############AAAAAAAAAAAAAAAAUUUUUUUUUUUUUUUUUUUUUUUURRRRRRRRRRRRRRRRRRRRRRRRAAAAAAAAAAAAAAAAAAAAAAAA
class aura():
    ...
    def start():
        ...
        try:
            ...
            sms.send(phone, 'HI, I just woke up ;) . You need anything?')
            time.sleep(5)
            sms.stored()
            led.off()
            return 1
